...
class PurchaseHistoryController extends Controller
{
    ...
    public function detailscode($id)
    {
        $order_query = Order::where('code', $id);
        return new PurchaseHistoryCollection($order_query->get());
    }
    ...
    public function itemsbycode($code){
        $order_id = Order::select('id')->where('code', $code)->pluck('id');
        $order_query = OrderDetail::where('order_id', $order_id);
        return new PurchaseHistoryItemsCollection($order_query->get());
    }
}
