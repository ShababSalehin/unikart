...
class ChatController extends Controller
{
    ...
    public function seller_create_conversation(Request $request)
    {
        
        $shop_details = Shop::findOrFail($request->seller_id);
        $conversation = new Conversation;
        $conversation->sender_id = $request->user_id;
        $conversation->receiver_id =$shop_details->user_id;
        $conversation->title = $request->title;
        $conversation->save();
        ...
        if($conversation->save()) {
            ...
            'shop_logo'=> api_asset($shop_details->logo),
            'title'=> $conversation->title,
            'message' => "Conversation created",]);
    }
}
