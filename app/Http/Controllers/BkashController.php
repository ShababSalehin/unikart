...
if($request->id){
$code =$request->id;
return view('backend.sales.refund',compact('code'));
}else{
return back();
}
    ...
    $code = '';
        ...
        if($result_data_array['refundTrxID']){
        Order::where('code', $request->orderCode)
       ->update([
           'bkash_refundtrxid' => $result_data_array['refundTrxID']
        ]);
        }
            return View('backend.sales.refund',compact('code'))->with('result_data_array', $result_data_array);
        }catch(Exception $e){
            return View('backend.sales.refund')->with('errorMessage', "Something Went Wrong !!");
