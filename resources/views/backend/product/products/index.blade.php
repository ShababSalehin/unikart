    ...
    <form class="" id="sort_products" action="" method="GET">
        <div class="card-header row gutters-5">
            ...
            <div class="col-md-2 ml-auto">
                <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" name="lowstock" id="lowstock" onchange="sort_products()">
                    <option value="">{{ translate('Low Stock') }}</option>
                    <option value="lowstock" @if($lowstock =='lowstock')selected @endif>{{translate('Low Stock (0-5)')}}</option>
                </select>
            </div>

            <div class="col-md-2 ml-auto">
                <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" name="discount_in" id="discount_in" onchange="sort_products()">
                    <option value="">{{ translate('Discount Ends In') }}</option>
                    <option value="7">{{translate('Discount Ends(1-7 days)')}}</option>
                    <option value="30">{{translate('Discount Ends(8-31 days)')}}</option>
                </select>
            </div>
            ...
            <div class="col-md-2 ml-auto">
                <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" name="type" id="type" onchange="sort_products()">
                    ...
                    <option value="created_at,desc"@isset($col_name , $query) @if($col_name == 'created_at' && $query == 'desc') selected @endif @endisset>{{translate('Sort By "Last Added')}}</option>
            ...
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        @endif
                        <th data-breakpoints="lg">{{translate('Discount Ends In')}}</th>
                        ...
                        <th data-breakpoints="lg">Products For You</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @if($product->discount_end_date == 0)
                            <td>{{ $discountexpair }}Days</td>
                        @else
                           
                          @if($product->discount_end_date < $today)
                          <td>{{$discountexpair}}Days</td>
                          @else
                          <td>
                           {{daysDiff($product->discount_end_date)}}
                          </td>
                          @endif
                       ...
                       <td>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_product_for_you(this)" value="{{ $product->id }}"
                                    type="checkbox" <?php if ($product->product_for_you == 1) {
                                        echo 'checked';
                                    } ?>>
                                <span class="slider round"></span>
                            </label>
                        ...
                        <td class="text-right">
                            ...
                            @if ($type == 'Seller')
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('products.seller.edit', ['id'=>$product->id, 'lang'=>env('DEFAULT_LANGUAGE','en')] )}}" title="{{ translate('Edit') }}">
                                <i class="las la-edit"></i>
                            </a>
                            @else
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('products.admin.edit', ['id'=>$product->id, 'lang'=>env('DEFAULT_LANGUAGE','en')] )}}" title="{{ translate('Edit') }}">
                                <i class="las la-edit"></i>
                            </a>
                            @endif
...
@section('script')
    <script type="text/javascript">
        ...
        function update_product_for_you(el) {
            if (el.checked) {
                var status = 1;
            } else {
                var status = 0;
            }
            $.post('{{ route('products.product_for_you') }}', {
                _token: '{{ csrf_token() }}',
                id: el.value,
                status: status
            }, function(data) {
                if (data == 1) {
                    AIZ.plugins.notify('success', '{{ translate('Products for you updated successfully') }}');
                } else {
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
            });
