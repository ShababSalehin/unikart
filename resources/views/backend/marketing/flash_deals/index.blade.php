            ...
            <tbody>
                @foreach($flash_deals as $key => $flash_deal)
                    <tr>
                        ...
                        <td>
							<label class="aiz-switch aiz-switch-success mb-0">
								<input onchange="update_flash_deal_freeshipping(this)" value="{{ $flash_deal->id }}" type="checkbox" <?php if($flash_deal->freeshipping_check == 1) echo "checked";?> >
								<span class="slider round"></span>
							</label>
						</td>
						<td>{{ url('campaign/'.$flash_deal->slug) }}</td>
						<td class="text-right">
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('flash_deals.edit', ['id'=>$flash_deal->id, 'lang'=>env('DEFAULT_LANGUAGE','en')] )}}" title="{{ translate('Edit') }}">
                          ...
                          -->
                            <a class="btn btn-soft-info btn-icon btn-circle btn-sm" href="{{route('flash_deals.reports', $flash_deal->id)}}" title="{{ translate('Reports') }}">
                            <i class="las la-mars-double"></i>
                            </a>
                        </td>
                    </tr>
                @endforeach
...
@section('script')
    <script type="text/javascript">
        ...
        function update_flash_deal_freeshipping(el){
            if(el.checked){
                var freeshipping_check = 1;
            }
            else{
                var freeshipping_check = 0;
            }
            $.post('{{ route('flash_deals.update_freeshipping') }}', {_token:'{{ csrf_token() }}', id:el.value, freeshipping_check:freeshipping_check}, function(data){
                if(data == 1){
                    location.reload();
                }
                else{
