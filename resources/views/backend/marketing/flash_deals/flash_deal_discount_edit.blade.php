    ...
    <thead>
      <tr>
        ...
        <td data-breakpoints="lg" width="15%">
            <span>{{translate('Seller Discount')}}</span>
        </td>
    ...
    <tbody>
        @foreach ($product_ids as $key => $id)
            ...
            <tr>
                ...
                <td>
                <?php if(isset($flash_deal_product->discount_type)){ ?>
                    <input <?php if(Auth::user()->user_type == 'seller') echo "readonly" ?> type="number" lang="en" name="unikart_discount_{{ $id }}" value="{{ $flash_deal_product->unikart_discount }}" min="0" step="1" class="form-control" >
                <?php }else{ ?> 
                    <input <?php if(Auth::user()->user_type == 'seller') echo "readonly" ?> type="number" lang="en" name="unikart_discount_{{ $id }}" value="" min="0" step="1" class="form-control" >
                <?php } ?>       
                </td>   
                 ...
                 <td>
                <?php if(isset($flash_deal_product->discount_type)){ ?>
                    <input <?php if(Auth::user()->user_type == 'seller') echo "readonly" ?> type="number" lang="en" name="app_discount_{{ $id }}" value="{{ $flash_deal_product->app_discount }}" min="0" step="1" class="form-control" >
                <?php }else{ ?> 
                    <input <?php if(Auth::user()->user_type == 'seller') echo "readonly" ?> type="number" lang="en" name="app_discount_{{ $id }}" value="" min="0" step="1" class="form-control" >
                <?php } ?>       
                </td>   
