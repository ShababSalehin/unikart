...
<form  id="culexpo" class="" action="" method="GET">
        <div class="card-header row gutters-5">
            <div class="col text-center text-md-left">
                <h5 class="mb-md-0 h6">{{ translate('Coupon Report') }}</h5>
            </div>
                ...
                <div class="col-md-3">
                    ...
                    <select id="demo-ease" class="aiz-selectpicker select2" name="product_id" data-live-search="true">
                        ...
                        <option @php if($pro_sort_by == $prod->id) echo 'selected';@endphp
                ...
                <div class="col-md-3">
                    <label class="col-form-label">{{translate('Sort by Seller')}} :</label>
                    <select id="shop_id" class="aiz-selectpicker select2" name="shop_name" data-live-search="true">
                        <option value=''>All</option>
                        @foreach (DB::table('shops')->select('id','name')->get(); as $key => $shop)
                        <option @php if($shop_sort_by ==  $shop->name) echo 'selected';@endphp
                            value="{{$shop->name }}">{{$shop->name}}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="col-auto">
                <div class="form-group mb-0">
                <button class="btn btn-sm btn-primary" onclick="submitForm ('{{ route('coupon_report_details') }}')">{{ translate('Filter') }}</button>
    ...
    <div class="card-body printArea">
        ...
        <h3 style="text-align:center;">{{translate('Coupon Report Details')}}</h3>
