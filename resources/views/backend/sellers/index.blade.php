                ...
                @foreach($sellers as $key => $seller)
                    @if($seller->user != null && $seller->user->shop != null)
                        <tr>
                            ...
                            @php
$totalpay =0;
$total_due_to_seller = \App\OrderDetail::where(['seller_id'=>$seller->user->id,'delivery_status'=>'delivered','refund_status'=>0])->sum('due_to_seller');
//dd($seller->user->id);
$total_payment = \App\Payment::where(['seller_id'=>$seller->user->id])->sum('amount');
$totalpay = $total_due_to_seller - $total_payment;
@endphp
{{single_price($totalpay)}}
                                <!-- @if ($seller->admin_to_pay >= 0)
                                    {{ single_price($seller->admin_to_pay) }}
                                @else
                                    {{ single_price(abs($seller->admin_to_pay)) }} (Due to Seller)
                                @endif -->
                            </td>
                            <td>
                                <div class="dropdown">
