    ...
    <div class="card">
        ...
        <div class="card-body">
          <form action="{{ route('sellers.update', $seller->id) }}" method="POST">
                ...
                <div class="form-group row">
                    <label class="col-sm-3 col-from-label" for="name">{{translate('Shpping Cost')}}</label>
                    <div class="col-sm-9">
                        <input type="number" placeholder="{{translate('Shpping Cost')}}" id="shipping_cost" name="shipping_cost" class="form-control" value="{{$shop->shipping_cost}}" required>
                    </div>
                </div>
