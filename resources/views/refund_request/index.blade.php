            ...
            <tbody>
                @foreach($refunds as $key => $refund)
                    <tr>
                        ...
                        <td>
                           <select class="form-control" id="refundid_{{$refund->id}}" onchange="changeRefundStatus({{$refund->id}})">
                                <option @if($refund->refund_status==0) {{'selected'}} @endif value="0">Requested</option>
                                <option @if($refund->refund_status==3) {{'selected'}} @endif value="3">Return In Progress</option>
                                <option @if($refund->refund_status==4) {{'selected'}} @endif value="4">Return Picked</option>
                           </select>
                        </td>
                        <td>
                        ...
                        <td class="text-right">
                            ...
                            <a class="btn btn-soft-danger btn-icon btn-circle btn-sm" onclick="reject_refund_request('{{ route('reject_reason_show', $refund->id )}}', '{{$refund->id}}', '{{ $refund->order_code }}')"  title="{{ translate('Reject Refund Request') }}">
...
@section('script')
    <script type="text/javascript">
        ...
        function changeRefundStatus(id){
            $.post('{{ route('status_change') }}',{_token:'{{ @csrf_token() }}', id:id,status:$('#refundid_'+id).val()}, function(data){
                if (data == 1) {
                    AIZ.plugins.notify('success', '{{ translate('Status has been done successfully') }}');
                }
                else {
