...
<div class="aiz-titlebar mt-2 mb-4">
      <div class="row align-items-center">
          <div class="col-md-6">
              <b class="h5">{{ translate('Conversations')}}</b>
          </div>
          <div class="col-md-4">
					<form class="" id="filter_conversation" action="" method="GET">
						<div class="input-group input-group-sm">
                            <label>Filter By Seller</label>
                            <select onchange="change_seller_filter()" class="aiz-selectpicker"data-live-search="true" data-placeholder="{{ translate('Select Seller')}}" name="search" id="search">
                            <option value="">{{ translate('Select Seller') }}</option>
                                @foreach($sellers as $seller)
                                    <option @if($sort_search==$seller->user_id) selected @endif value="{{$seller->user_id}}">{{$seller->user->name}}</option>
                                @endforeach
                            </select>
      ...
      <div class="card-body">
        <ul class="list-group list-group-flush">
          @foreach ($conversations as $key => $conversation)
              ...
              @php if(!empty($sort_search) && ($sort_search!=$conversation->receiver_id))
                       continue;     
              @endphp
