                    ...
                    <div class="shadow-sm bg-white p-3 p-lg-4 rounded text-left">
                        <div class="mb-4">
                            <div class="row gutters-5 d-none d-lg-flex border-bottom mb-3 pb-3">
                                ...
                                <div class="col fw-600">{{ translate('Discount')}}</div>
                                ...
                                @foreach ($carts as $key => $cartItem)
                                    @php
                                        ...
                                        if($cartItem['offer_status']==1){
                                            $disc=$cartItem['offer_discount_amount']+$cartItem['unicart_discount_amount'];
                                            $total = $total + (($product->unit_price + $cartItem['tax']) * $cartItem['quantity'])-$disc;
                                        }else{
                                    ...
                                    <li class="list-group-item px-0 px-lg-3">
                                        <div class="row gutters-5">
                                            ...
                                            <span class="mr-2 ml-0">
                                                  <img src="{{ uploaded_asset( getVarianImage($cartItem['product_id'],$cartItem['variation']))}}"
                                            ...
                                            <div class="col-lg col-4 order-2 order-lg-0 my-3 my-lg-0">
                                                <span class="opacity-60 fs-12 d-block d-lg-none">{{ translate('Discount')}}</span>
                                                <?php if($cartItem['offer_status']==1){ ?>
                                                    <span class="fw-600 fs-16">{{ single_price($cartItem['offer_discount_amount']+$cartItem['unicart_discount_amount']) }}</span>
                                                <?php }else{ ?>
                                                    <span class="fw-600 fs-16">{{ single_price($product->unit_price-$cartItem['price']) }}</span>
                                                <?php } ?>    
                                            </div>
                                            ...
                                            <div class="col-lg col-6 order-4 order-lg-0">
                                                @if($cartItem['digital'] != 1 && $product->auction_product == 0)
                                                    <div class="row no-gutters align-items-center aiz-plus-minus mr-2 ml-0">
                                                        ...
                                                        <input type="number" name="quantity[{{ $cartItem['id'] }}]" class="col border-0 text-center flex-grow-1 fs-16 input-number" placeholder="1" value="{{ $cartItem['quantity'] }}" min="{{ $product->min_qty }}" max="{{ !empty($product_stock->qty) ? $product_stock->qty : 1 }}" onchange="updateQuantity({{ $cartItem['id'] }}, this)">
                                            ...
                                            <div class="col-lg col-4 order-3 order-lg-0 my-3 my-lg-0">
                                                ...
                                                <?php if($cartItem['offer_status']==1){ ?>
                                                    <span class="fw-600 fs-16 text-primary">{{ single_price(($product->unit_price* $cartItem['quantity'])-$disc) }}</span>
                                                <?php }else{ ?>
        ...
        <div class="modal-dialog modal-dialog-zoom">
            <div class="modal-content">
                ...
                <div class="text-center pt-4">
                    ...
                    <div class="p-3">
                    <form class="form-default" role="form" action="{{ route('otplogin') }}" method="POST">
                        @csrf
                        <div class="form-group">
                            
                        <input type="text" class="form-control" placeholder="ENTER 11 DIGIT PHONE NUMBER" name="phone" id="phone">

                        </div>

                        <div class="mb-3">
                            <button type="submit" class="btn btn-primary btn-block fw-600">{{  translate('Login/Sign up') }}</button>
                        </div>
                    </form>
                    ...
                    @endif
                    <div class="text-center">
                <p class="text-muted mb-0">{{ translate('already registered with E-mail?')}}</p>
                <a href="{{ route('login_with_email') }}" class="btn btn-primary btn-block fw-600">{{ translate('Login With Email')}}</a>
            </div>
                </div>
