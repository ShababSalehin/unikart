...
<meta property="product:sale_price:amount" content="{{ str_replace(' ৳', '', home_discounted_price($detailedProduct)) }}" />
@endif
...
<meta property="product:item_group_id" content="{{$detailedProduct->category->name ?? ""}}">
 ...
 <ul class="breadcrumb bg-transparent p-0">
        <li class="breadcrumb-item opacity-50">
            ...
            <li class="text-dark fw-600 breadcrumb-item">
                <a class="text-reset" href="{{url()->current()}}">"{{ $detailedProduct->getTranslation('name') }}"</a>
            </li>
            ...
            <div class="row">
                    ...
                    <div class="sticky-top z-3 row gutters-10">
                        ...
                        <div class="col order-1 order-md-2">
                            <div class="aiz-carousel product-gallery" data-nav-for='.product-gallery-thumb'
                                ...
                                @endforeach
                                @foreach ($detailedProduct->stocks->sortByDesc('qty') as $key => $stock)
                                @if ($stock->image != null)
                                <div class="carousel-box img-zoom rounded">
                                    <img style="width:70% !important;max-width:70% !important" class="img-fluid lazyload"
                                        src="{{ static_asset('assets/img/placeholder.jpg') }}"
                        ...
                        @php 
                            $varientnumber = count($detailedProduct->stocks);
                            if($varientnumber > 5){
                                $item = 5;
                            }else{
                                $item = $varientnumber;
                        ...
                        <div class="col-12 col-md-auto w-md-80px order-2 order-md-1 mt-3 mt-md-0">
                            @if(\App\ProductStock::where('product_id',$detailedProduct->id)->get()->count() > 1)
                            <div class="aiz-carousel product-gallery-thumb" data-items='{{$item}}'
                            @else
                            ...
                            @endif
                                ...
                                @foreach ($detailedProduct->stocks->sortByDesc('qty') as $key => $stock)
                                @if ($stock->image != null)
                                <div onclick="selectCaraosalBox('{{ $stock->variant }}')"  class="carousel-box c-pointer border p-1 rounded"
                ...
                <div class="col-xl-4">
                    <div class="text-left">
						...
						<div style="font-size:1.0rem;"><span id="product_description"></span></div>
                        ...
                        <hr>
                            ...
                            <div class="row align-items-center">
                                <div class="col-auto">
                                    ...
                                    <a href="{{ route('shop.visit', $detailedProduct->user->shop->slug) }}"
                                        class="text-reset">{{ substr($detailedProduct->user->shop->name, 0, 12) }}</a>
                                    @else
                            ...
                            <form id="option-choice-form">
                                ...
                                @foreach (json_decode($detailedProduct->choice_options) as $key => $choice)
                                    @php
                                if($choice->attribute_id==16)
                                	continue;
                                
                                    $a_publish_check=\App\Attribute::find($choice->attribute_id);
                                    if($a_publish_check->publish_status!=1){
                                      //continue; 
                                    	$display="none";
                                    	$dclass=" d-none";
                                    }else{
                                ...
                                <div class="row no-gutters {{$dclass}}">
                                   <div class="col-sm-2">
                                       <div class="opacity-50 my-0">{{ \App\Attribute::find($choice->attribute_id)->getTranslation('name') }}:</div>
                                    </div>
                                   <div class="col-sm-10">
                                       <?php if(\App\Attribute::find($choice->attribute_id)->getTranslation('name')=='Size'){ ?> 
                                                ...
                                                @foreach ($detailedProduct->stocks->sortByDesc('qty') as $key => $stock)
                                                    ...
                                                    @endphp
                                                        <label class="aiz-megabox pl-0 mr-2">
                                                            <input type="radio" name="attribute"  id="variantchange_{{ $stock->variant }}"
                                                                value="{{ $stock->variant }}"
                                                                @if($key == 0) checked @endif >
                                                            
                                                                <span class=" aiz-megabox-elem rounded d-flex align-items-center justify-content-center py-1 px-1 mb-1">
                                        ...
                                        <?php }else{ ?>
                                         ...
                                         <div class="aiz-radio-inline">
                                                @foreach ($detailedProduct->stocks->sortByDesc('qty') as $key => $stock)
                                                    @if ($stock->image != null)
                                                    <label class="aiz-megabox pl-0 mr-2">
                                                        <input type="radio" name="attribute"  id="variantchange_{{ $stock->variant }}" value="{{ $stock->variant }}"
                                                                @if($stock->qty > 0) checked @endif >
                                                                <span class=" aiz-megabox-elem rounded d-flex align-items-center justify-content-center py-1 px-1 mb-1">
                                                                <img style="width:30px;height:30px;" class="lazyload mw-100 size-50px mx-auto"
                                                                src="{{ static_asset('assets/img/placeholder.jpg') }}"
                                                                data-src="{{ uploaded_asset($stock->image) }}"
                                                                onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}';">
                ...
                <div class="col-xl-3 bg-secondary-custom pd-right-section">
                    <div class="pd-right-header w-100 pt-3">
                    @if ($detailedProduct->added_by == 'seller' && $detailedProduct->user->seller != null)
                    <button class="btn btn-sm btn-primary text-left line-height-1 pd-right-but-padding"><span style="font-size: 8px;" >{{ translate('Sold by')}}</span><br><span style="font-size:12px">{{ substr($detailedProduct->user->shop->name, 0, 12) }}</span></button>
                    @else
                    <button style="font-size: 12px;" class="btn btn-sm btn-primary text-left line-height-1 pd-right-but-padding"><span style="font-size: 8px;" >{{ translate('Sold by')}}</span><br><span style="font-size:12px">{{ translate('Inhouse product')}}</span></button>
                    @endif
                                
                                @if (get_setting('conversation_system') == 1)
                                                
                                                    <button class="btn btn-sm btn-primary float-right"
                                                        onclick="show_chat_modal()" style="padding:10px">{{ translate('Message Seller')}}</button>
                                                
                                                @endif
                    </div>
                  <hr>
                  @if ($detailedProduct->added_by == 'seller' && $detailedProduct->user->seller != null)
                  <div class="w-100 pd-right-header">
                 
                    <button style="background-color: #FFF;" class="btn btn-sm  btn-link line-height-1 pd-right-but-padding"><span style="font-size: 9px;" >{{ translate('Positive seller Rating')}}</span><br>
                                {{ get_positive_seller_ratting($detailedProduct->user->seller->user_id) }} %</button>
                   <a href="{{ route('shop.visit', $detailedProduct->user->shop->slug) }}"><button style="background-color: #FFF;padding:10.5px" class="btn btn-sm  btn-light  float-right">{{ translate('Visit Store')}}</button></a>
                  ...
                  <div class="row no-gutters mt-2">
                                <div class="col-sm-3">
                                    <div class="opacity-50 my-2 h6 small">{{ translate('Return')}}:</div>
                                </div>
                                <div class="col-sm-9">
                                <a href="{{ route('returnpolicy') }}" class="ml-2 float-right"
                                        target="_blank">{{ translate('View Policy') }}</a>
                                    <b>3 days easy return</b><br>
                  ...
                  <div class="row no-gutters mt-2">
                                <div class="col-sm-3">
                                    <div class="opacity-50 my-2 h6 small">{{ translate('Delivery Time')}}:</div>
                                </div>
                                <div class="col-sm-9">
                                    2-3 Days inside Dhaka City,<br>
                                        3-5 Days Outside Dhaka City
                                </div>
                            </div>
                            <hr>
                                     <div class="row no-gutters mt-2">
                                <div class="col-sm-3">
                                    <div class="opacity-50 my-2 h6 small">{{ translate('Shipping Cost')}}:</div>
                                </div>
                  ...
                  <div class="row no-gutters mt-2">
                                <div class="col-sm-3">
                                    <div class="opacity-50 my-2 h6 small">{{ translate('Share')}}:</div>
                                </div>
                                <div class="col-sm-9">
            ...
            <div class="col-xl-3 order-1 order-xl-0">
                ...
                <div class="bg-white rounded shadow-sm mb-3">
                    <div class="p-3 border-bottom fs-16 fw-600">
                        <h2 class="fs-16 fw-600 mb-0">{{ translate('Top Selling Products')}}</h2>
                    </div>
                    <div class="p-3">
                        <ul class="list-group list-group-flush">
                            @foreach (filter_products(\App\Product::where('user_id',
                            $detailedProduct->user_id)->orderBy('num_of_sale', 'desc'))->limit(6)->get() as $key =>
                            $top_product)
                            <li class="py-3 px-0 list-group-item border-light">
                                <div class="row gutters-10 align-items-center">
                                    <div class="col-5">
                                        <a href="{{ route('product', $top_product->slug) }}" class="d-block text-reset">
                                            <img style="object-fit: contain;" class="img-fit lazyload h-xxl-110px h-xl-80px h-120px"
                                                src="{{ static_asset('assets/img/placeholder.jpg') }}"
            ...
            <div class="col-xl-9 order-0 order-xl-1">
                <div class="bg-white mb-3 shadow-sm rounded">
                    ...
                    <a href="#tab_default_1" data-toggle="tab"
                            class="p-3 fs-16 fw-600 text-reset active show"><h2 class="fs-16 fw-600 mb-0">{{ translate('Description')}}</h2></a>
                        <a href="#tab_default_5" data-toggle="tab"
                            class="p-3 fs-16 fw-600 text-reset"><h2 class="fs-16 fw-600 mb-0">{{ translate('Specification')}}</h2></a>
                        @if($detailedProduct->video_link != null)
                        <a href="#tab_default_2" data-toggle="tab"
                            class="p-3 fs-16 fw-600 text-reset"><h2 class="fs-16 fw-600 mb-0">{{ translate('Video')}}</h2></a>
                        @endif
                        @if($detailedProduct->pdf != null)
                        <a href="#tab_default_3" data-toggle="tab"
                            class="p-3 fs-16 fw-600 text-reset"><h2 class="fs-16 fw-600 mb-0">{{ translate('Downloads')}}</h2></a>
                        @endif
                        <a href="#tab_default_4" data-toggle="tab"
                            class="p-3 fs-16 fw-600 text-reset"><h2 class="fs-16 fw-600 mb-0">{{ translate('Reviews')}}</h2></a>
                    </div>
                        ...
                        <div class="tab-pane fade" id="tab_default_5">
                            <div class="p-4">
                                <div class="mw-100 overflow-hidden text-left aiz-editor-data">
                                    ...
                                    @if(!empty($detailedProduct->beauty_features))

                                    <div class="mw-100 overflow-hidden text-left aiz-editor-data">
                                        <div class="row no-gutters">
                                            <div class="col-sm-2">
                                                <div class="opacity-50 my-2">Beauty Feature:</div>
                                            </div>
                                            <div class="col-sm-10">
                                                <div class="my-2">
                                                    {{$detailedProduct->beauty_features}}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            <div class="bg-white rounded shadow-sm">
                ...
                <div class="p-3">
                    <div class="aiz-carousel gutters-5 half-outside-arrow" data-items="5" data-xl-items="3"
                        ...
                        data-infinite='true'>
                        @foreach (filter_products(\App\Product::where('category_id',
                            ...
                            <div class="aiz-card-box border border-light rounded hov-shadow-md my-2 has-transition">
                                <div class="">
                                    <a href="{{ route('product', $related_product->slug) }}" class="d-block">
                                        <img style="object-fit: contain;" class="img-fit lazyload mx-auto h-140px h-md-250px"
                                            src="{{ static_asset('assets/img/placeholder.jpg') }}"
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-zoom product-modal" id="modal-size" role="document">
        <div class="modal-content position-relative">
            ...
            <form class="" action="{{ route('conversations.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="">
                    <img class="img-fluid lazyload" style="width:20%" src="{{ static_asset('assets/img/placeholder.jpg') }}" data-src="{{ uploaded_asset($photos[0]) }}" onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}';">
                    </div>
                    <div class="form-group">
...
function selectCaraosalBox(variant){
$("input[name=attribute]").each(function(i,v){
	if($(v).val()==variant){
    	($(v).prop('checked',true));
    	getVariantPrice();
    }
...
<script type = "text/javascript">
        ...
        dataLayer.push({
        	...
        	ecommerce: {
            	items: [{
                    item_name 	: "{{$detailedProduct->name}}", // Name or ID is required.
...
<script type="application/ld+json">
{
  ...
  "image": "{{ uploaded_asset($photo) }}",
  "description": "{{ strip_tags($detailedProduct->description) }}",
  "sku": "00000000",
  "gtin8": "00000000",
  "brand": "{{$detailedProduct->brand->name ?? ""}}",
  "offers": {
    "@type": "Offer",
    "price": "{{ filter_var(home_discounted_price($detailedProduct), FILTER_SANITIZE_NUMBER_FLOAT) }}",
 ...
 @if(count($detailedProduct->reviews) > 0)
  ...
  @foreach ($detailedProduct->reviews as $key => $review)
   ...
   @if(count($detailedProduct->reviews)-1 == $key)
    }
  @else
  },
  @endif
