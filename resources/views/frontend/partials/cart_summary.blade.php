<div class="card border-0 shadow-sm rounded">
        ...
        <table class="table">
            ...
            <tbody>
                @php
                    ...
                    $shipping_region = !empty($shipping_info['city']) ? $shipping_info['city'] : '';
                @endphp
                @foreach ($carts as $key => $cartItem)
                    @php
                        $product = \App\Product::find($cartItem['product_id']);
                        $subtotal += $product->unit_price * $cartItem['quantity'];
						$shippingsubtotal += $cartItem->price * $cartItem['quantity'];
                    ...
                    <tr class="cart_item">
                        <td class="product-name">
                            ...
                            <strong class="product-quantity">
                                @php echo "(" @endphp {{ $product->unit_price }}× {{ $cartItem['quantity'] }} @php echo ")" @endphp
                            </strong>
                        </td>
                        <td class="product-total text-right">
                            <span class="pl-4 pr-0">{{ single_price($product->unit_price* $cartItem['quantity']) }}</span>
                        </td>
                    </tr>
                @endforeach
    ...
    if($shipping_skip_total > $shippingsubtotal){
        //echo $shipping;
    }else{
            $shipping = 0;
            }
        ...
        <table class="table">
            ...
            <tfoot>
                ...
                <tr class="cart-shipping">
                    ...
                    <td class="text-right">
                     @if($shipping == 0)
                        ...
                        @else
                          @if(isset($areawiseshipping))
                          <span id="shipping_cost" class="font-italic">{{ $areawiseshipping }}</span>
                          @else
                          <span id="shipping_cost" class="font-italic">0</span>
                          @endif
                        @endif
                    </td>
                </tr>
                ...
                <tr class="cart-total">
                    <th><span class="strong-600">{{translate('Total')}}</span><input type="hidden" id="totalpri" value="{{$total}}"></th>
                    <td class="text-right">
                    @if($shipping == 0)
                        <strong><span>{{ single_price($total) }}</span></strong>
                        @else
                         @if(isset($areawiseshipping))
                         <strong><span id="totalprittl">{{ single_price($total + $areawiseshipping) }}</span></strong>
                         @else
                         <strong><span id="totalprittl">{{ single_price($total) }}</span></strong>
                         @endif
            ...
            @else
                <div class="mt-3">
                    <form class="" id="apply-coupon-form" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="areawiseshipping" id="areawiseshipping">
                        <input type="hidden" name="owner_id" value="{{ $carts[0]['owner_id'] }}">
                        <div class="input-group">
