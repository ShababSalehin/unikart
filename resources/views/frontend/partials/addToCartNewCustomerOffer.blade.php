...
@php 
                                    ...
                                    if($flash_deal->status == 1 && strtotime(date('Y-m-d H:i:s')) <= $flash_deal->end_date){
                                        $flash_deal_product = App\FlashDealProduct::where('product_id',$product->id)->where('flash_deal_id',$flash_deal->id)
                                        ->first();
                                        }
                                        if($flash_deal_product)
                                            $net_price=$product->unit_price-($flash_deal_product->discount+$flash_deal_product->unikart_discount);
                                        else
                                            $net_price=home_discounted_price($product);
                                   @endphp
                @if($net_price != home_discounted_price($product))
                        ...
                        <div class="col-10">
                            <div class="">
                                <strong class="h2 fw-600 text-primary">
                                   ...
                                   <span class="fw-700 text-primary">৳{{ $net_price }}</span>
                                </strong>
                            </div>
                        </div>
                    </div>
                                        ...
                                        @foreach ($product->stocks as $key => $stock)
                                            @if ($stock->image != null)
                                                <label class="aiz-megabox pl-0 mr-2">
                                                    <input
                                                            type="radio"
                                                    name="attribute" id="variantchange_{{ $stock->variant }}"
                                                            value="{{ $stock->variant }}"
                            ...
                            <div class="col-10">
                                <div class="product-quantity d-flex align-items-center">
                                    <div class="row no-gutters align-items-center aiz-plus-minus mr-3" style="width: 130px;">
                                        ...
                                        <input type="text" name="quantity" class="col border-0 text-center flex-grow-1 fs-16 input-number" placeholder="1" value="{{ $product->min_qty }}" min="{{ $product->min_qty }}" max="{{ $product->max_qty}}">
